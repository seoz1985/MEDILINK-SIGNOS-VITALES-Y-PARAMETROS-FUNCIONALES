...
export function BottomNav({ disabled }: { disabled?: boolean }) {
  ...
  return (
    <nav className={`fixed bottom-0 left-0 right-0 z-50 border-t border-border bg-card/95 backdrop-blur-md safe-area-inset-bottom ${disabled ? 'pointer-events-none opacity-50' : ''}`}>
      <div className="max-w-lg mx-auto flex items-center justify-around px-2 py-2">
        {items.map((item) => {
          ...
          return (
            <button
              key={item.view}
              onClick={() => { if (!disabled) navigate(item.view) }}
              disabled={disabled}
